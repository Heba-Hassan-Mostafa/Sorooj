...
@section('title')
    {{ trans('dashboard.sidebar.books-comments') }}
@endsection
@section('content')
    <h4 class="py-3">
    ...
    <div class="card">
        <div class="table-responsive p-2">
            <table class="myDatatable table table-bordered">
                ...
                <tr>
                    <th>#</th>
                    <th>{{ trans('dashboard.books.client_name') }}</th>
                    <th>{{ trans('dashboard.books.email') }}</th>
                    <th>{{ trans('dashboard.books.comment') }}</th>
                    <th>{{ trans('dashboard.books.stars') }}</th>
                    <th>{{ trans('dashboard.books.comment-belongs-to') }}</th>
                ...
                @foreach ($comments as $comment)
                    <tr>
                        ...
                        <td>{{ $comment->commentable != '' ? $comment->commentable?->book_name : '----' }}</td>
                        <td>
                        ...
                        <td>
                            <div class="m-2">
                                ...
                                <form action="{{ route('admin.books.comments.destroy', $comment->id) }}" method="POST"
                                      id="form-{{ $comment->id }}">
                                    @csrf
                                    @method('Delete')
                                </form>
                            ...
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5 fw-bold" id="exampleModalLabel">
                                        {{ trans('dashboard.books.show_comment') }}</h1>
        ...
        $(function() {
            ...
            $('#DataTables_Table_0').on('change','.status',  function() {
                ...
                $.ajax({
                    ...
                    url: '{{ route('admin.books.comments.change-status') }}',
