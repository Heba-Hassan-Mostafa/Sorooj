...
@section('content')
    ...
    <a href="{{ route('admin.blogs.category.create') }}" class=" mainBtnStyle me-2 mb-2">
        {{ trans('dashboard.blogs.add-blog-category') }}
    </a>
    <a href="{{ route('admin.blogs.category.sort-categories') }}" class="btn btn-warning font-weight-bold p-2"
                    ...
                    <div class="card accordion-item active">
                        <h2 class="accordion-header d-flex align-items-center">
                            <button
                                ...
                                {{trans('dashboard.blogs.parent-categories')}}
                            </button>
                        </h2>
                                ...
                                <table class="myDatatable table table-bordered">
                                    ...
                                    @foreach ($categories as $category)
                                        ...
                                        <span class="switch-off">
                                            ...
                                            <td>
                                                <div class="m-2">
                                                    <a href="{{ route('admin.blogs.category.edit',$category->id) }}"  class="btn btn-info btn-sm text-white"  title="{{ trans('dashboard.edit') }}">
                                                    ...
                                                    <form action="{{ route('admin.blogs.category.destroy', $category->id) }}" method="POST"
                                                          id="form-{{ $category->id }}">
                                                        @csrf
                                                        @method('Delete')
                                                    </form>
                    ...
                    <div class="accordion-item card">
                        <h2 class="accordion-header d-flex align-items-center">
                            <button
                                ...
                                {{trans('dashboard.blogs.blog-subcategories')}}
                            </button>
                        </h2>
                            ...
                            <div class="accordion-body">
                                <table class="myDatatable table table-bordered">
                                    ...
                                    <tr>
                                        ...
                                        <th>{{ trans('dashboard.blogs.blog-count') }}</th>
                                        <th>{{ trans('dashboard.created_at') }}</th>
                                        <th>{{ trans('dashboard.blogs.parent-category') }}</th>
                                        <th>{{ trans('dashboard.status') }}</th>
                                        <th>{{ trans('dashboard.options') }}</th>
                                    ...
                                    @foreach ($subCategories as $subCategory)
                                        <tr>
                                            ...
                                            <td>{{ $subCategory?->blogs?->count() ?? 0 }}</td>
                                        ...
                                        <span class="switch-off">
                                            ...
                                            <td>
                                                <div class="m-2">
                                                    <a href="{{ route('admin.blogs.category.edit',$subCategory->id) }}"  class="btn btn-info btn-sm text-white"  title="{{ trans('dashboard.edit') }}">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
                                                    ...
                                                    <form action="{{ route('admin.blogs.category.destroy', $subCategory->id) }}" method="POST"
        ...
        $(function() {
            ...
            $('.myDatatable').off('change', '.status').on('change', '.status', function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.blogs.category.change-status') }}',
                    data: {
                        'status': status,
                        'id': id
                    },
