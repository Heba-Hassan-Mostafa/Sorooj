...
@section('content')
    ...
    <a href="{{ route('admin.videos.category.create') }}" class=" mainBtnStyle me-2 mb-2">
        {{ trans('dashboard.videos.add-video-category') }}
    </a>
    <a href="{{ route('admin.videos.category.sort-categories') }}" class="btn btn-warning font-weight-bold p-2"
    ...
    <div class="card mt-3">
        <div class="table-responsive p-2">
            <table class="myDatatable table table-bordered">
                ...
                <tr>
                    <th>#</th>
                    <th>{{ trans('dashboard.videos.video-category-name') }}</th>
                    <th>{{ trans('dashboard.videos.video-count') }}</th>
                ...
                @foreach ($categories as $category)
                    <tr>
                        ...
                        <td>
                            <div class="m-2">
                                <a href="{{ route('admin.videos.category.edit',$category->id) }}"  class="btn btn-info btn-sm text-white"  title="{{ trans('dashboard.edit') }}">
                                ...
                                <form action="{{ route('admin.videos.category.destroy', $category->id) }}" method="POST"
                                      id="form-{{ $category->id }}">
                                    @csrf
                                    @method('Delete')
                                </form>
        ...
        $(function() {
            $('.myDatatable').off('change', '.status').on('change', '.status', function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.videos.category.change-status') }}',
                    data: {
                        'status': status,
                        'id': id
                    },
