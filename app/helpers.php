...
if (!function_exists('sendOtpMail')) {
    function sendOtpMail($code, $email)
    {
        try{
            ...
            Log::error("Error in sending otp mail: {$e->getMessage()}");
            return false;
        }
    ...
    if (!function_exists('displayCategories')) {
        function displayCategories($categories, $depth = 0) {
            foreach ($categories as $category) {
                // Indent categories based on depth (level of nesting)
                echo '<option value="' . $category->id . '">' . str_repeat('--', $depth) . ' ' . $category->name . '</option>';

                // Check if the category has subcategories and recursively call the function
                if ($category->children) {
                    displayCategories($category->children, $depth + 1); // Increase depth for subcategories
                }
            }
        }
    }
